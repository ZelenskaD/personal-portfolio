            ...
            <div className="relative container mx-auto px-6 text-center">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 py-16 gap-8">
                    {skills.map((skill, index) => (
                        <div
                            key={index}
                            className="skill-card p-4 bg-white/10 rounded-lg shadow-lg hover:scale-110 hover:bg-gradient-to-r from-purple-500 via-pink-500 to-purple-600 hover:shadow-1xl transition-all duration-300"
                        >
                            <img
                                src={skill.icon}
                                alt={skill.name}
                                className="mx-auto w-16 h-16 mb-4 transition-transform duration-300 hover:rotate-12"
                            />
